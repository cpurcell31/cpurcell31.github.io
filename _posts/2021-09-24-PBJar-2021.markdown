...
Based on the webserver's response, the script uses narrows down the search area.
Eventually, the script will narrow the field down to the last available version number.
...
we can see there are two varieties of interrupt in frames - input and confirmation. Within the input frames
...
Both are completely valid ways to solve this problem but for "wow factor" and to save some eye-strain I'll demo
a script for reading and decoding the frames. However, first, I needed to export only the keyboard
...
The script is created with the format of these specific frames in mind. Each frame consists
of 72 bytes with the first 8 being a constant header of "c0 cc 0d 66 c5 98 ff ff".
Using regex the script finds all blocks matching these specific requirements.

The extracted blocks are then trimmed down to the keyboard input code. Aside some special keys like
shift and ctrl, the data for key strokes are located in the last 6 bytes. When a key is pressed
...
Since there are 6 bytes, for most keyboards a maximum of 6 keys can be pressed at a time.
...
mentioned earlier, the script decodes the code values into ascii characters. You can find a full
...
radio operators to send pictures. But more on a more interesting note, it has been used quite a bit for
...
The example is should remind you of the original .wav file for this challenge. Now that we know what we are dealing with,
...
to get a dump of all the hashes from the file. To do this I use the hashdump plugin.  

![Hashdump](/assets/pbjar/meme2.png)
<figcaption align=center><b>Figure 18-2: Volatility - Hashdump Results</b></figcaption>
&nbsp;

The Mimikatz plugin also works even better for this task and skips a bit of work cracking the hashes.
Unfortunately, the plugin broke on my machine. Anyway, after a few seconds 3 hashes popped out.
Since this is a Windows capture we are working with, all the hashes should be in the NTLM format.
...
memory. The goal of this challenge is to find the name of someone who sent a phishing email.
