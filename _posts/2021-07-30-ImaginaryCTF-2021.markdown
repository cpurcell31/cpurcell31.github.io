...
In a sort of "spur of the moment" way, I decided to join my first CTF competition. This one was hosted by the team over
at [Imaginary CTF](https://imaginaryctf.org/) and was a blast! I went in with very little knowledge and I ended up impressed
...
The simplest proof of concept to check for SSTI is to input `{{7*7}}` and if the application displays 49 then it's vulnerable.
...
then be used in conjunction with `__subclasses__()` to find a list of all objects.
    ...
    {{""__class__.__mro__.__subclasses_()}}

However, back in the Python code there is a blacklist which bans 3 peculiar things: "las", "bas", and "bal". Which stops
the use of things like class, subclasses, and global. Luckily, using some annoying concatenation this was avoidable. The
equivalent injection was

```
{{""["__cla"+"ss__"].__mro__[1]["__subcla"+"sses__"]()}}  
```
...
```
{{""["__cla"+"ss__"].__mro__[1]["__subcla"+"sses__"]()[99]("flag.txt", "flag.txt").get_data("flag.txt")}}
```
...
![CBC](/assets/2021-ICTF/cbc-mode-decrypt.jpg)
...
this was as simple as counting to the right index and adding or subtracting 1 from the value located there. For example:
    ...
    {HEX VALUE HERE} -> {NEW HEX VALUE HERE}
...
First, I copied the words into a program like googledocs and then manually scanned through the list for each misspelling
...
The blacklist then removes the only 'or' it sees and sends the result off to the next step. Here is the statement after
...
and firepwny:pwned. I logged in as firepwny and checked got to see Rick Astley.
...
the nonce can be forced to be a value we know. This means we can force F(k,n) to essentially be the same value every time.
...
![Cookie Surgery](/assets/2021-ICTF/cookie-surgery.png)
...
double as many times as the previous.
    ...
    iccttttffffffff{{{{{{{{{{{{{{{{00000000000000000000000000000000nnnnnnnnnnnnnnnnnnnnnnnnnnn [...]
